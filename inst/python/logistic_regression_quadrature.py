...
def _logreg_quad_fixed_intercept_jax(x, y, b, b_weights, b0):
    conditional = conditional_log_likelihood_vec_b_jax(x, y, b, b0)
    marginal = logsumexp(conditional + jnp.log(b_weights))
    return marginal

@jit
def logreg_quad_fixed_intercept_jax(x, y, b_mu, b_sigma, b0, nodes, weights):
    b_nodes = scale_nodes(nodes, b_mu, b_sigma)
    return _logreg_quad_fixed_intercept_jax(x, y, b_nodes, weights, b0)
...
def b0_fixed_point_iteration(val):
    ...
    return (b0, x, sumy, b, delta)

def b0_converged(val):
    return val[-1] < 1e-4

def best_conditional_log_likelihood_jax(x, y, b):
    # step 1: optimize b0 by simple fixed point method
    b0_init = logit(jnp.mean(y))
    val_init = (b0_init, x, jnp.sum(y), b, 1)
    ...
    return conditional_log_likelihood_jax(x, y, b, b0)

best_conditional_log_likelihood_jax_b_vec = vmap(
    best_conditional_log_likelihood_jax,
    (None, None, 0), 0
)

def _logreg_quad_best_intercept_jax(x, y, b, b_weights):
    conditional = best_conditional_log_likelihood_jax_b_vec(x, y, b)
    marginal = logsumexp(conditional + jnp.log(b_weights))
    return marginal

@jit
def logreg_quad_best_intercept_jax(x, y, b_mu, b_sigma, nodes, weights):
    """
    x, y 1d data
    ...
    """
    b_nodes = scale_nodes(nodes, b_mu, b_sigma)
    logp = _logreg_quad_best_intercept_jax(x, y, b_nodes, weights)
    return logp
...
def logreg_quad_jax(x, y, b_mu, b_sigma, b0_mu, b0_sigma, nodes, weights):
    ...
    logp = _logreg_quad_jax(x, y, b_nodes, weights, b0_nodes, weights)
    return logp

# vectorized over columns of X
logreg_quad_X_jax = jit(vmap(
    logreg_quad_jax,
...
logreg_quad_sigma_jax = jit(vmap(
    logreg_quad_jax,
